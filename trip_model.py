...
class TripModel:
    ...
    # LLM을 활용한 여행 일정 추천
    def generate_initial_schedule(self, data):
        days = data["days"]
        places = data["places"]
    ...
    def parse_llm_schedule(self, schedule):
        parsed_schedule = []
    ...
    def optimize_schedule_with_distance(self, parsed_schedule, places):
        # 장소 정보를 위도/경도와 매칭 (객체 속성 접근 방식)
        ...
        for day in range(1, max(p["day"] for p in parsed_schedule) + 1):
            ...
            for perm in permutations(remaining_places):
                ...
                total_distance = sum(
                    self.haversine(route[i].latitude, route[i].longitude,
...
# ------# ------
    ...
    # json 형식으로 바꿈 
    def convert_to_join(self, schedule_list):
        records = []
        day_counter = {}
